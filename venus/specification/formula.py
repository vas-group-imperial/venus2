...
# Description: Formula class and subclasses.
# ************
...
class Formula:
    ...
    class Sense:
        (LT, GT, NE) = ('<', '>', '!=')
        (LE, GE, EQ) = ('<=', '>=', '==')
        inverted = {LE: GT, GT: LE, GE: LT, LT: GE, EQ: NE, NE: EQ}
...
class NegationFormula(UnaryFormula):
    ...
    def to_NNF(self):
        ...
        elif isinstance(subformula, TrueFormula):
            return FalseFormula()
        elif isinstance(subformula, VarVarConstraint):
            return VarVarConstraint(subformula.op1, Formula.Sense.inverted[subformula.sense], subformula.op2)
        elif isinstance(subformula, VarConstConstraint):
            return VarConstConstraint(subformula.op1, Formula.Sense.inverted[subformula.sense], subformula.op2)
        elif isinstance(subformula, LinExprConstraint):
            return LinExprConstraint(subformula.op1, Formula.Sense.inverted[subformula.sense], subformula.op2)
        elif isinstance(subformula, ConjFormula):
            return DisjFormula(NegationFormula(subformula.left).to_NNF(), NegationFormula(subformula.right).to_NNF())
        elif isinstance(subformula, DisjFormula):
            return ConjFormula(NegationFormula(subformula.left).to_NNF(), NegationFormula(subformula.right).to_NNF())
        elif isinstance(subformula, NAryDisjFormula):
            return NAryConjFormula([NegationFormula(clause).to_NNF() for clause in subformula.clauses])
        elif isinstance(subformula, NAryConjFormula):
            return NAryDisjFormula([NegationFormula(clause).to_NNF() for clause in subformula.clauses])
        elif isinstance(subformula, ENextFormula):
            return ANextFormula(subformula.k, NegationFormula(subformula.left).to_NNF())
        elif isinstance(subformula, ANextFormula):
            return ENextFormula(subformula.k, NegationFormula(subformula.left).to_NNF())
        elif isinstance(subformula, EUntilFormula):
            """
            NOT(E phi1 U^k phi2) =>
        ...
        elif isinstance(subformula, AUntilFormula):
            """
            NOT(A phi1 U^k phi2) =>
